...
describe('NotificationModal', () => {
  ...
  it('uses English announcement content and updates when the locale changes', async () => {
    ...
    expect(wrapper.text()).toContain('維護通知')
    setLocale('en')
  ...
  it('falls back to canonical announcement content when English fields are missing or blank', () => {
    setLocale('en')
    const wrapper = mount(NotificationModal, {
      props: {
        visible: true,
        summary: {
          announcements: [{ ...summary.announcements[0], title_en: null, body_en: '   ' }],
          personal_notifications: [],
          counts: { total: 1 },
        },
      },
