...
| `AboutUsEntry` | Independent persisted bilingual Markdown bodies, compatibility title metadata, a persisted display order, and an optional last-editor reference | Authenticated-readable bilingual managed information with Chinese locale fallback; Markdown owns visible headings, administrators control sequence, and permanent deletion bypasses Trash | Confirmed by code and focused API tests |
| `HomepageSloganSubmission` | Optional submitter and reviewer references use `SET NULL`; a required submitter-name snapshot preserves moderation context | One plain-text homepage slogan candidate with independent review state and occurrence level; terminal administrator deletion removes only this row and bypasses Trash | Confirmed by code, additive migration, and focused API/schema tests |
...
| `ArchiveWishReport` | Optional Wish/reporter/reviewer/deleting-administrator FKs use `SET NULL`; snapshots preserve the reported target; independent soft delete | Moderation record survives Wish or User deletion, moves once from pending to upheld/dismissed, and uses the same Trash lifecycle as `CommentReport` | Confirmed by API and schema contracts |
...
### Stage 5F-A durable recovery relationships

The inert [permanent-deletion contract](permanent-deletion.md) adds an
operation → logical target → exact object identity hierarchy without making
the recovery records children of a live Archive, submission, Course, or other
deletable entity. Logical target identity therefore survives deletion of the
live row. A deletion-safe requester reference uses `ON DELETE SET NULL` and
retains no actor snapshot. Storage identity remains a removable child record,
so completed minimal audit need not retain bucket/key/version recovery data
for the full 180-day operation-audit period.

These empty tables do not change the existing ArchiveSubmissionEvent detach,
live-row deletion transaction, Archive placement, or lifecycle ownership.
