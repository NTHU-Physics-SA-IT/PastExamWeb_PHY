...
| `SystemIssueReport` | Optional reporter, immutable canonical user-authored title/description, nullable administrator-authored English presentation metadata, read/review metadata, GitHub-sync metadata, independent soft delete | Operational report independent of archive/submission lifecycle; translation metadata never replaces or machine-translates the reporter's original | Confirmed by code and tests |
| `Notification` / announcement receipts | Global canonical Chinese announcement with nullable English presentation metadata plus per-user read receipt | Site-wide announcement, distinct from a personal event notification; locale presentation does not change activation or read lifecycle | Confirmed by code and tests |
