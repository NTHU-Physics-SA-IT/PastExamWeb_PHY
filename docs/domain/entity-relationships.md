...
| `ArchiveWish` | Creator-owned immutable exam target snapshot with a unique canonical `target_key`; optional Course FK uses `SET NULL` | Persists a Past Exam Wish without creating a Course or Category. Fulfillment is derived from a matching effective-public Archive and does not hide the wish | Confirmed by API and migration tests |
