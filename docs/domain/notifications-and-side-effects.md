...
conflict without changing either report or enqueuing a notification. A
successful trash releases active uniqueness only with its commit; a successful
restore or create occupies it in that same caller-owned transaction. The named
partial unique index remains the integrity-race fallback.
Revalidation failure therefore leaves Report, Archive or Submission, and
...
| Archive metadata mutation/reparent | Edit-only, move-only, and combined edit/move use one shared mutation boundary; the route owns administrator authorization, canonical source/target Course, Archive, and exact Submission locks, post-lock validation, mutation, and one commit | Silent and database-only; it creates/restores no Course, changes no Submission, performs no MinIO operation, and rolls back both metadata and placement on failure |
| Linked ArchiveSubmission reapproval | The approval route preserves the exact Archive `course_id`, copies corrected non-placement snapshot fields, transitions status, and enqueues notification in one caller-owned transaction | It does not resolve/create proposed parents or emit a separate transfer event; rollback retains prior Archive placement/metadata and submission state |
