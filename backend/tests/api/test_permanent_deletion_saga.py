...
@pytest.mark.asyncio
@pytest.mark.skipif(
    not os.getenv("STAGE5FE_REAL_MINIO_ENDPOINT"),
    reason="requires the task-owned Stage 5F-E MinIO container",
)
async def test_real_minio_reconciler_verifies_unknown_outcome_before_completion(
    session_maker,
    make_user,
) -> None:
    endpoint = os.environ["STAGE5FE_REAL_MINIO_ENDPOINT"]
    access_key = os.environ["STAGE5FE_REAL_MINIO_ACCESS_KEY"]
    secret_key = os.environ["STAGE5FE_REAL_MINIO_SECRET_KEY"]
    bucket_name = os.environ["STAGE5FE_REAL_MINIO_BUCKET"]
    real_client = Minio(
        endpoint,
    ...
    uploaded = real_client.put_object(
        bucket_name,
        archive.object_name,
        BytesIO(b"stage5fe-real-versioned-object"),
        len(b"stage5fe-real-versioned-object"),
    ...
    try:
        async with session_maker() as session:
            operation = await accept_permanent_deletion(
                ...
                root_entity_type=TrashEntityType.ARCHIVE,
                root_entity_id=int(archive.id),
                idempotency_key=f"stage5fe:real:{archive.id}",
                requested_by_user_id=requester.id,
                storage=storage,
                now=now,
            )
