...
def test_reviewed_manifest_registry_has_required_revisions() -> None:
    assert HEAD_SCHEMA_REVISION == "c7e4a9b2d6f1"
    ...
    assert get_manifest_spec("c7e4a9b2d6f1").metadata_variant == "head"
...
def test_model_derived_manifest_variants_are_cumulative_and_isolated() -> None:
    ...
    head = metadata_for_revision("c7e4a9b2d6f1")
    ...
    pre_sibling_branches = metadata_for_revision("e6a1b3c5d7f9")
    ...
    assert pre_sibling_branches is not None
    assert "archive_wishes" in head.tables
    assert "archive_wishes" in main_head.tables
    assert "archive_wishes" not in coordination_head.tables
    assert "archive_wishes" not in pre_sibling_branches.tables
    ...
    for lifecycle_column in (
        ...
        assert lifecycle_column in coordination_head.tables["course_submissions"].c
        assert lifecycle_column not in main_head.tables["course_submissions"].c
        assert (
            lifecycle_column
    ...
    rebuilt_head = metadata_for_revision("c7e4a9b2d6f1")
