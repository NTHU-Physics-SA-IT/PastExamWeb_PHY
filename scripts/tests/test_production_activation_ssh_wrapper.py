...
@pytest.mark.parametrize(
    ...
    [
        "status",
        f"observe {SHA} 123 1",
        f"diagnose-class-zero {SHA} 123 1",
        ...
        "reconcile-activation activation-123-1",
...
@pytest.mark.parametrize(
    ...
    [
        ...
        f"preflight {SHA} 0 1",
        f"observe {SHA} 123 0",
        f"observe {SHA} 123 1 extra",
        f"observe {'A' * 40} 123 1",
        f"observe {SHA} '123;id' 1",
        f"observe {SHA} 123 1\nid",
        f"diagnose-class-zero {SHA} 0 1",
        f"diagnose-class-zero {SHA} -1 1",
        f"diagnose-class-zero {SHA} run 1",
        f"diagnose-class-zero {SHA} 123 0",
        f"diagnose-class-zero {SHA} 123 nope",
        f"diagnose-class-zero {'A' * 40} 123 1",
        f"diagnose-class-zero {SHA} 123 1 extra",
        f"diagnose-class-zero {SHA} '123;id' 1",
        "request-status activation-123-1;id",
        "reconcile-activation activation-123-1 extra",
        "reconcile-activation ../activation-123-1",
        "bash",
        "sh -c id",
    ],
